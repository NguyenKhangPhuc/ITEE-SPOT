/**
 * PURPOSE:
 * Renders the Specific Criteria grading list (02_SPECIFIC_CRITERIA) with range sliders.
 *
 * CONTEXT/PARENT FILE:
 * Sibling component of SubmissionGradingClient.tsx, located at 'app/submission/grading/[submissionId]/components/SpecificCriteriaSection.tsx'.
 *
 * INPUTS / PARAMETERS:
 ...
 * - specificGrades (UserSubmissionGradeWithPercentage[], Required): Watch array of specific criteria grades.
 */
...
export default function SpecificCriteriaSection({
  specificCriteria,
  register,
  specificGrades,
}: SpecificCriteriaSectionProps) {
  if (specificCriteria.length === 0) return null
    ...
    <div className={`${tw.bg.surfaceContainerLow} ${tw.border.whiteSubtle} border rounded-sm p-6 flex flex-col gap-6 relative overflow-hidden select-none`}>
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-3">
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-[#e8e1df] uppercase tracking-wider">
          <span className="material-symbols-outlined text-sm text-[#00e0b3]">award_star</span>
          <span>02_SPECIFIC_CRITERIA</span>
        </div>
      ...
      <div className="flex flex-col gap-5">
        {specificCriteria.map((field, index) => {
          const currentVal = specificGrades?.[index]?.grade ?? 50
