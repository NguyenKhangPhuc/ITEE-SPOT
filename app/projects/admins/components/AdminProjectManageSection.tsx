/**
 * PURPOSE:
 * Renders the administrator projects management registry table.
 * Allows admins to filter/sort all system projects, update validation statuses,
 * and edit details in place.
 *
 * CONTEXT/PARENT FILE:
 * Child component of ProjectsAdminClient.tsx, located at 'app/projects/admins/components/AdminProjectManageSection.tsx'.
 *
 * INPUTS / PARAMETERS:
 * - page (string, Required): Active view indicator.
 * - currentProjects (ProjectsSummary[], Required): Preloaded list of projects.
 * - setCurrentProjects (React.Dispatch, Required): State setter for projects list.
 */
...
export default function AdminProjectManageSection({
  ...
  const handleFilterProjectStatus = async (status: PROJECT_STATUS | null): Promise<void> => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        showNotification("Failed to filter project database.")
      }
    } finally {
  ...
  const handleFilterProjectOrder = async (ascending: boolean): Promise<void> => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        showNotification("Failed to sort project database.")
      }
    } finally {
      setIsOpenLoader(false)
    }
  ...
  /**
   * BEHAVIORAL MECHANISM:
   * Asserts status tag text colors.
   *
   * PARAMETERS:
   * - status (string | null): The project status string.
   *
   * RETURNS:
   * - string: Tailwind classes.
   */
  ...
  /**
   * BEHAVIORAL MECHANISM:
   * Triggers status change write queries to database.
   *
   * PARAMETERS:
   * - projectId (string): Target project ID.
   * - status (PROJECT_STATUS): New validation status selection.
   *
   * RETURNS:
   * - Promise<void>
   */
  const handleUpdatingProjectsStatus = async ({
    projectId,
    status,
  }: {
    projectId: string
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        showNotification("Failed to update project status.")
      }
    } finally {
      setIsOpenLoader(false)
    }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 select-none">
          <div className="flex items-center gap-2 text-xs font-mono font-bold text-[#e8e1df] uppercase tracking-wider">
            ...
            <span>01_GLOBAL_PROJECTS_DATABASE</span>
          </div>
          ...
          <table className="w-full border-collapse font-mono text-[10px] text-[#b9cbc2] text-left min-w-[900px]">
            <thead>
              <tr className="border-b border-white/5 bg-[#151312] text-[#83958d] select-none text-[8.5px] uppercase tracking-wider">
                ...
                <th className="p-4 font-bold text-center w-36">SET_STATUS</th>
            ...
            <tbody>
              {currentProjects.length > 0 ? (
                  ...
                  >
                    ...
                    <td className="p-4 text-center w-36">
                      <div className="relative flex items-center w-full">
                        <select
                          onChange={(e) =>
                            handleUpdatingProjectsStatus({
                              projectId: item.id!,
                              status: e.target.value as PROJECT_STATUS,
                            })
                          }
                          value={item.project_status ?? PROJECT_STATUS.PENDING}
                          className="bg-[#151312] text-[#e8e1df] border border-white/5 font-mono text-[9px] p-2 pr-6 rounded-sm outline-none focus:border-[#00e0b3]/50 transition-colors w-full appearance-none cursor-pointer"
                        >
                ...
                <tr>
                  <td colSpan={8} className="p-12 text-center text-[#83958d] select-none">
                    NO PROJECT ENTRIES REGISTERED IN SYSTEM
                  </td>
                </tr>
              )}
            </tbody>
