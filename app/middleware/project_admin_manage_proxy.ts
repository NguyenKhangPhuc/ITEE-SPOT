...
export async function projectsManageRoute({ request, user }: { request: NextRequest, user: User | null }) {
    ...
    if (
        ...
        if (specificRoute == 'admins') {
            ...
            const { data: projectData, error: projectError } = await supabase.from('projects').select('id,project_status').eq('id', specificRoute).maybeSingle();
            if (projectError) {
                const url = request.nextUrl.clone()
